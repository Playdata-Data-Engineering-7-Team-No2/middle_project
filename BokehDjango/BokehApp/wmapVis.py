...
# In[ ]:
# 날씨 데이터 plot 
def weathermapPlot(lat, lng, wDf, zoom=10, map_type='roadmap'):
        ...
        <div>
            <div>
                <img
                    ...
                    style="float: left; margin: 40px 80px 40px 80px;"
                    border="2"
                ></img>
            </div>
            <div>
                <span style="font-size: 17px; font-weight: bold;">지명</span>
            ...
            <div>
                ...
                <span style="font-size: 15px; color: #966;">@wind m/s</span>
            </div>   
            <div>
    ...
    p.triangle(x = 'lon', y = 'lat', size=10, alpha=0.6,  source=weatherSrc, legend_label='날씨', color='blue')
...
def main():
    ...
    # 지도 데이터 
    sql = "select * from weather.current_weather as w INNER JOIN weather.location as l ON w.name = l.name;"
    mapDf = pd.read_sql_query(sql,conn_db)

    lat , lon = 33.4,126.6 # 제주도 중앙 경도 위도 
    gmPlot = weathermapPlot(lat, lon, mapDf, map_type='roadmap', zoom=9) # 바다 데이터 추가하기 
    # show(gmPlot)
    conn_db.close()
    return gmPlot
